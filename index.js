...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const cy = cytoscape({
        ...
        style: [
            ...
            { selector: '.start-state', style: { 'background-color': '#76c7c0' } }, // Different color for start state
            { selector: '.final-state', style: { 'background-color': '#f8b400' } } // Different color for final states
        ],
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const cy = cytoscape({
        ...
        style: [
            { selector: 'node', style: { 'label': 'data(label)', 'text-valign': 'center', 'color': '#000', 'background-color': '#ccc', 'width': '60px', 'height': '60px', 'font-size': '12px' } },
            { selector: 'edge', style: { 'label': 'data(label)', 'width': 2, 'line-color': '#000', 'target-arrow-color': '#000', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier' } },
            { selector: '.start-state', style: { 'background-color': '#00ff00' } }, // Different color for start state
            { selector: '.final-state', style: { 'background-color': '#0000ff' } } // Different color for final states
        ],
        layout: { name: 'grid', rows: 1 }
    });
    ...
    faForm.addEventListener("submit", (event) => {
        ...
        automaton.states = statesInput.value.split(',').map(s => s.trim());
        automaton.alphabet = alphabetInput.value.split(',').map(s => s.trim());
        automaton.transitions = parseTransitions(transitionsInput.value);
        automaton.startState = startStateInput.value.trim();
        automaton.finalStates = finalStatesInput.value.split(',').map(s => s.trim());
    ...
    document.getElementById("test-string").addEventListener("click", () => {
        const inputString = document.getElementById("test-string-input").value.trim();
        const result = testString(inputString, automaton);
        faDisplay.innerText = result ? `String "${inputString}" is accepted.` : `String "${inputString}" is rejected.`;
    });
    ...
    // Function to render the automaton diagram using Cytoscape with animation
    function renderAutomaton(fa) {
        cy.elements().remove(); // Remove existing elements from Cytoscape
        const elements = convertAutomatonToCytoscapeElements(fa); // Convert FA to Cytoscape elements
        cy.add(elements).animate({ // Add new elements to Cytoscape with animation
            style: { 'opacity': 1 },
            duration: 500
        });
    ...
    function convertAutomatonToCytoscapeElements(fa) {
        ...
        fa.states.forEach(state => {
            elements.push({
                ...
                classes: `${state === fa.startState ? 'start-state' : ''} ${fa.finalStates.includes(state) ? 'final-state' : ''}`
            });
        });

        fa.transitions.forEach(({ from, symbol, to }) => {
            elements.push({
                data: { source: from, target: to, label: symbol }
            });
        });
